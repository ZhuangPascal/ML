...
class RegLin:
    ...
    def fit(self, xtrain, ytrain, niter=100, gradient_step=1e-5):
        ...
        #On commence avec des paramètres positives pour l'exemple
...
class NonLin2:
    ...
    def SGD(self, net, loss, xtrain, ytrain, batch_size=1, niter=1000, gradient_step=1e-5):
        ...
        for i in range(niter):
            ...
            self.list_loss.append(np.mean(loss.forward(ytrain, self.optim.predict(xtrain).reshape(-1,1)) ))
...
class AutoEncodeur :
    ...
    def fit(self, loss, xtrain, output, batch_size=1, neuron=10, niter=1000, gradient_step=1e-5):
        """ Classifieur non-linéaire sur les données d'apprentissage.
            @param xtrain: float array x array, données d'apprentissage
            @param ytrain: int array, labels sur les données d'apprentissage
            @param batch_size: int, taille des batchs
        ...
        # Initialisation des couches du réseau
        self.linear_1 = Linear(input, neuron)
        self.tanh = TanH()
        self.linear_2 = Linear(neuron, output)
        self.sigmoide = Sigmoide()
        ...
        for i in range(niter):
            ...
            if(i%100==0):
                print("Erreur itération",i,":",np.sum(loss.forward(xtrain, res_forward[-1])))
                
            # Phase backward (rétro-propagation du gradient de la loss
            #          par rapport aux paramètres et aux entrées)

            deltas =  [loss.backward( xtrain, res_forward[-1] ) ]
...
def arti_data(data_type = 1):
    # Création de données artificielles suivant 4 gaussiennes
    datax, datay = gen_arti(epsilon=0.1, data_type=data_type)
...
def tsne(datax, datay, perplexity_list=[5, 10, 30, 50, 100]):
    ...
    for i in range(len(perplexity_list)):
        ...
        plt.scatter(tsne_perp_data[i][:,0],tsne_perp_data[i][:,1], c=datay)
        plt.colorbar()
...
def mainLineaire(a=97.28, sigma=50):
    """
        Test de la partie linéaire : régression linéaire.
    """
    ...
    rl.fit(xtrain, ytrain.reshape(-1,1), niter=1000, gradient_step=1e-5)
...
def mainNonLineaire(neuron = 4, niter=100, gradient_step=1e-3, data_type=1):
    """
        Tests de la partie non-linéaire sur les données gen_arti.
    """
    
    #---------------------------- Non Automatise ------------------------------
    
    # Création de données artificielles suivant 4 gaussiennes
    datax, datay = arti_data(data_type=data_type) 
    
    # Création et test sur un réseau de neurones non linéaire
    time_start = time.time()
    
    nonlin = NonLin()
    nonlin.fit(datax, datay, niter=niter, neuron=neuron, gradient_step=gradient_step)
    
    # Test sur les données d'apprentissage
    ypred = nonlin.predict(datax)
    datay = np.argmax(datay, axis=1)
    
    print("Regression non Linéaire :")
    print("Temps d'exécution: ", time.time() - time_start, "secondes")
    print("Score de bonne classification: ", np.mean( ypred == datay ))
    plot(datax, datay, nonlin, name='Regression non Linéaire')
    ...
    nonlin = NonLin2()
    nonlin.SGD(net, mse, datax, datay, batch_size=len(datay), niter=niter, gradient_step=gradient_step)
    
    # Test sur les données d'apprentissage
    ypred = nonlin.predict(datax)
    datay = np.argmax(datay, axis=1)
    
    print("\nRegression non Linéaire automatisé :")
    print("Temps d'exécution: ", time.time() - time_start, "secondes")
    print("Score de bonne classification: ", np.mean( ypred == datay ))
    plot(datax, datay, nonlin, name='Regression non Linéaire automatisé')

def mainMultiClasse(loss=CESM(), bruit=False, neuron = 100, classes=10, niter=100, gradient_step=0.01):
    """
        Tests de la partie multi-classe sur les données manuscrites (usps).
    """
    # Chargement des données USPS
    xtrain, ytrain, xtest, ytest = usps_data(classes=classes, a_bruit=bruit)
    ...
    nonlin = NonLin2()
    nonlin.SGD(net, loss, xtrain, ytrain, batch_size=len(ytrain), niter=niter, gradient_step=gradient_step)
    
    # Test sur les données d'apprentissage
    ypred = nonlin.predict(xtest)
    
    plt.figure()
    plt.title(f"Evolution de la fonction coût {type(loss).__name__}")
    plt.plot(nonlin.list_loss, label='Loss')
    plt.legend()
    print(f"\nMulti-classe avec {type(loss).__name__}, bruit={bruit} :")
    print("Temps d'exécution: ", time.time() - time_start, "secondes")
    print("Score de bonne classification: ", np.mean( ypred == ytest ))
    
def mainAutoEncodeur(loss=BCE(), bruit=False, neuron = 100, classes=10, niter=500, gradient_step=1e-4):
    """
    ...
    time_start = time.time()
    print(f"Auto-encodeur avec {type(loss).__name__}, bruit={bruit}, {niter} itérations :")
    
    # pour auto-encodeur
    auto = AutoEncodeur()
    auto.fit(loss, xtrain, ytrain.shape[1], niter=niter, neuron=neuron, gradient_step=gradient_step)
    
    # Test sur les données de tests
    ...
    plt.savefig(f"auto_encodeur_{type(loss).__name__}_{bruit}.pdf")
    plt.show()  
       
    #Affichage t-sne sur les données reconstruites
    tsne(y_deco, pred_deco)
    
    print("\nTemps d'exécution: ", time.time() - time_start, "secondes")
...
if (__name__ == "__main__"):
    ...
    # mainLineaire()
    # mainLineaire(a=10, sigma=0.5)
    
    
    #-------- Tests de la partie non-linéaire sur les données gen_arti --------
    
    # mainNonLineaire(neuron=20, niter=100, gradient_step=1e-3, data_type=1)
    ...
    # mainMultiClasse(loss=MSELoss(), bruit=bruit, neuron = 100, classes=10, niter=n_iter, gradient_step=gradient_step)
    # mainMultiClasse(loss=CESM(), bruit=bruit, neuron = 100, classes=10, niter=n_iter, gradient_step=gradient_step)
    ...
    mainAutoEncodeur(loss=MSELoss(), bruit=bruit, neuron = neuron, classes=10, niter=n_iter, gradient_step=gradient_step)
    # mainAutoEncodeur(loss=CESM(), bruit=bruit, neuron = neuron, classes=10, niter=n_iter, gradient_step=gradient_step)
    # mainAutoEncodeur(loss=BCE(), bruit=bruit, neuron = neuron, classes=10, niter=n_iter, gradient_step=gradient_step)
    
    #---------------- t-SNE sur les données initiales de tests ----------------
    
    # xtrain, ytrain, xtest, ytest = usps_data(classes=10, a_bruit=False)
    # tsne(xtest, ytest)